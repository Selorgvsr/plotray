...
import { ChevronDown, TrendingUp, BarChart3, MapPin, Shield, Briefcase, Lock, ArrowRight, Sparkles } from "lucide-react";
        ...
        <div className={`flex flex-col ${reverse ? "lg:flex-row-reverse" : "lg:flex-row"} items-center gap-12`}>
          ...
          <div className="lg:w-1/2 space-y-6">
            ...
            <div className="space-y-3">
              {content.map((line, index) => <p key={index} className="text-foreground/80 text-lg leading-relaxed flex items-start gap-3">
            ...
            {/* Get More Button */}
            <Button onClick={() => setIsExpanded(!isExpanded)} className="bg-gradient-to-r from-plotray-emerald to-plotray-gold hover:from-plotray-emerald/90 hover:to-plotray-gold/90 text-white rounded-full px-8 py-3 font-semibold transition-all duration-300 shadow-lg hover:shadow-xl">
...
const CTAExpandable = () => {
  const [isExpanded, setIsExpanded] = useState(false);
  const expandedContent = <>Building a land portfolio requires strategic planning Early entry increases appreciation potential Asset-worthy land grows with infrastructure Expert insights reduce investment risk Clear data supports confident decisions Long-term focus ensures stability <span style={{
      color: '#166534'
  ...
  return <div className="space-y-4">
      ...
      <Button onClick={() => setIsExpanded(!isExpanded)} variant="outline" className="border-white/30 text-white hover:bg-white/20 rounded-full px-8 bg-transparent">
        <span>{isExpanded ? "Show Less" : "Get More"}</span>
        <ChevronDown className={`ml-2 h-5 w-5 transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`} />
      </Button>
    </div>;
...
const LandAsset = () => {
  const sections = [{
    ...
    content: ["Land appreciates steadily as cities expand", "Limited availability increases long-term value", "Unlike buildings, land does not depreciate", "Urban growth boosts demand and pricing", "Land remains one of the safest investment assets"],
    ...
    expandedContent: <>Tracking land performance helps investors plan strategically Market movements become easier to understand with clear data Nearby infrastructure directly impacts appreciation rates <span style={{
        ...
        color: '#F97316'
      }}>RAY</span> visualizes these changes in a simple format Historical trends support future planning decisions Data-driven insights reduce uncertainty Clear analytics strengthen asset confidence Informed investors make stronger choices</>,
    ...
    content: ["Connectivity drives land appreciation", <><span style={{
        display: 'inline-block'
      }}><span style={{
          color: '#166534'
        }}>PLOT</span><span style={{
    ...
    content: ["Legal clarity protects your investment", "Clear titles increase buyer trust", "Proper zoning improves resale potential", "Documentation strengthens asset security", <><span style={{
        display: 'inline-block'
      }}><span style={{
          color: '#166534'
        }}>PLOT</span><span style={{
    ...
    content: ["Land supports long-term financial planning", "Ideal for wealth preservation and legacy creation", "Lower risk compared to volatile assets", "Urban growth strengthens returns", "A stable foundation for future wealth"],
...
It's a Growing Asset<br />
          ...
          <p className="text-xl md:text-2xl lg:text-3xl text-white/90 tracking-wide bg-[#00a846] py-[3px] px-0 font-bold my-0 mx-0">3X PORTFOLIO, FINANCIAL FREEDOM, KID'S FUTURE SECURED</p>
        </div>
      ...
      <section className="relative py-24 overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center" style={{
        backgroundImage: `url(${landassetCta})`
      }} />
        ...
        <div className="relative z-10 container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              <Button size="lg" className="bg-gradient-to-r from-plotray-gold to-amber-400 hover:from-plotray-gold/90 hover:to-amber-400/90 text-primary font-bold text-lg px-10 py-6 rounded-full shadow-xl hover:shadow-2xl transition-all duration-300">
                Start Managing Assets
                <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button size="lg" variant="outline" className="border-white/30 text-white hover:bg-white/20 rounded-full px-10 py-6 bg-transparent" asChild>
